...
**Amendment 2026-07-12 (Plan 5b-1): deck + arch backend as RPCs (migration 0014).**
- **Deck match creates no feud.** Mutual like lives in `swipes` only. Either matched party proposes concrete terms — `propose_feud(p_target, p_ordeal_id, p_mode, p_goal) returns uuid` creates a feud `status='proposed'` with new column `feuds.proposed_by uuid` (`on delete set null`); the other party answers via `respond_feud(p_feud_id, p_accept)` → `active` or `dissolved`. Proposer answering own glove → `not_thine_to_answer`. This concretizes the friends-first amendment's "deck feuds keep `proposed` for their setup step"; §4/§8 webhook-on-swipes phrasing superseded.
- `swipe_rival(p_target, p_liked) returns jsonb {matched}` — inserts the swipe (duplicate → `already_swiped`), reports mutual like. A block yields `matched=false` silently (stealth). `propose_feud` reports blocked pairs as `no_match` (stealth).
- `get_deck(max_cards int default 20) returns setof jsonb` — as §3, errors as exceptions: `location_required` when caller has no location/radius. `search_path = public, extensions` (PostGIS lives in `extensions` — §hardening 5). Excludes: self, swiped-by-caller, block either direction, any live/proposed feud with caller, no shared approved ordeal, outside either party's radius. Orders by distance.
- **Arch ops are RPCs** (forge_ordeal precedent; §4 EF rows for declare-arch/resolve-declare/dissolve-arch superseded):
  - `declare_arch(p_target) returns uuid` — errors: `self_declare`, `target_dead` (missing OR blocked — stealth), `arch_exists` (caller has active arch feud), `declare_pending` (live declare), `token_cooling` (`token_available_at > now()`).
  - `resolve_declare(p_declare_id, p_accept) returns uuid|null` — target only, pending only (`declare_dead`). Accept: `arch_exists` if either side already pacted; the **oldest active** feud between the pair becomes the pact (`is_arch=true, unmasked_at=now()`), else a new active arch feud ignites on the pair's first shared approved ordeal (`mode='endless'`); no shared ordeal → `no_shared_ordeal` (declare stays pending, target can still decline). Decline: `declined` + `token_available_at = now() + 30 days`.
  - `dissolve_arch(p_feud_id)` — either member, active arch only (`feud_dead`). Feud `dissolved` + `ended_at`; the accepted declare row → `dissolved` with declarer token cooldown 30 days. Dissolution ends `unmask_pact` visibility (policy requires `status='active'`).
- **`notify` EF gains profile-scoped kinds** `deck_match` and `declare`: input `{kind, target_profile_id}`; EF verifies the claimed relationship server-side (mutual liked swipes / pending declare by caller) before pushing. `push_match` copy reused for deck matches; `push_declare` for declares. Same fire-and-forget trade-off as Plan 4.
...
**Amendment 2026-07-12 (Plan 5c — migration 0017): measured ordeals + inactivity forfeit.**
- **`ordeals.aggregation text not null default 'sum' check in ('sum','latest')`** — resolves design-spec §17's open question. `sum` = deeds add up (all existing behavior). `latest` = level tracking (chess rating, max bench): towers and totals show each side's newest entry; showdown = first entry to REACH the goal (`new.value >= goal` in the trigger). Direction stays higher-is-better; lower-is-better remains unsupported.
- **`forge_ordeal` is now 4-arg**: `(p_name, p_unit, p_language, p_aggregation default 'sum')`; 3-arg signature dropped (PostgREST callers passing 3 args still resolve via the default). `bad_aggregation` on junk.
- **`forfeit_feud(p_feud_id) returns void`** — design-spec §14 forfeit; open question N resolved: **14 days**. Member-only, active feuds; eligible when the opponent's newest score entry (or the feud's `created_at` if they never logged) is 14+ days old; sets `status='ended'`, `winner = caller`, `ended_at`, atomic `status='active'` guard. Errors: `feud_dead`, `not_gone_soft`.
- **Gone-soft marker is client-derived** (same 14-day computation, active feuds only) — no schema flag. Feud cards and the feud screen show it; the feud screen offers the forfeit claim.

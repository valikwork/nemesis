...
- **Settings layout revision (2026-07-12, owner walk feedback):** top-left ← back arrow (no bottom Back button — it visually attached to the danger zone); "Carved." confirmation auto-dismisses after ~2.5s; brutality is a collapsed select (current tier name+description, chevron) opening a picker sheet — not five stacked rows; danger zone is last and contains only the erase action.
